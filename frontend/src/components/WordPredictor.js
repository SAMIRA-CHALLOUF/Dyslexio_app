...
const WordPredictor = React.memo(function WordPredictor({ 
  ...
  const handleCorrection = async () => {
    ...
    try {
      ...
      const result = data.correctedText || data;
      setCorrectedText(result);
      setCorrectionsList(data.corrections || []); // Store the corrections with explanations
  ...
  return (
    <div ref={panelRef} style={{ ...styles.panel, top, left }}>
      ...
      {/* ✅ Zone de correction — Texte cliquable + Explications */}
          ...
          <div style={styles.correctionZone}>
            ...
            {correctionsList && correctionsList.length > 0 && (
              <div style={styles.explanationsContainer}>
                <div style={styles.explanationsTitle}>Explications:</div>
                <ul style={styles.explanationsList}>
                  {correctionsList.map((c, i) => (
                    <li key={i} style={styles.explanationItem}>
                      <span style={styles.correctionOriginal}>{c.original}</span> ➔ <span style={styles.correctionFixed}>{c.corrected}</span>
                      <div style={styles.correctionRule}>{c.rule}</div>
                    </li>
                  ))}
                </ul>
              </div>
