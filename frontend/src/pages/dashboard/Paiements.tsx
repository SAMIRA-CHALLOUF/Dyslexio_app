...
export default function Paiements() {
  ...
  const exportCSV = () => {
    const rows = [
      ["Prénom", "Nom", "Classe", "Niveau", "Parent", "Référence", "Méthode", "Montant ($)", "Statut", "Date paiement"],
      ...eleves.map((e) => [
    ...
    <DashboardLayout>
      ...
      <div style={layout.grid4}>
        ...
        <KpiCard
          ...
          sub={`${stats.enAttente.reduce((s, e) => s + e.paiement.montant, 0).toLocaleString("fr-FR")} $`}
        />
        <KpiCard
          ...
          sub={`${stats.retard.reduce((s, e) => s + e.paiement.montant, 0).toLocaleString("fr-FR")} $`}
        />
      </div>
                    ...
                    >
                      ...
                      <td style={{ ...layout.td, textAlign: "right" }}>
                        <span style={{ fontWeight: 800, fontSize: 14, color: e.paiement.statut === "payé" ? colors.teal : colors.amber }}>
                          {e.paiement.montant.toLocaleString("fr-FR")} $
                        </span>
                      </td>
                    </tr>
                  ))}
