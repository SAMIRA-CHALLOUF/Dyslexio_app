...
export default function Navbar({ isLoggedIn, user, onLogin, onSignUp, onLogout }) {
  ...
  // ✅ selectedLang suit i18n.language
  ...
  // ✅ Change la langue dans i18n ET met à jour l'affichage
  ...
  return (
    <nav style={{
      ...
      background: scrolled ? 'var(--navbar-bg, rgba(253,251,245,0.95))' : 'transparent',
      ...
      <div style={{
        ...
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none' }}>
          ...
          <div>
            <div style={{ fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: '1.1rem', color: 'var(--text-primary, #1A1A2E)' }}>
              {t('nav.brand')}
            </div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted, #888)', lineHeight: 1 }}>{t('nav.tagline')}</div>
          </div>
        </Link>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
          {NAV_LINKS.map((l) => {
            if (l === 'nav.editor' && !isLoggedIn) return null;
            if (l === 'nav.editor') {
              return (
                <Link key={l} to="/editor"
                  style={{ fontSize: '0.9rem', color: 'var(--text-primary, #444)', textDecoration: 'none', fontWeight: 500, transition: 'color 0.2s' }}
                  onMouseOver={(e) => (e.target.style.color = TEAL)}
                  onMouseOut={(e) => (e.target.style.color = 'var(--text-primary, #444)')}
                >{t('nav.editor')}</Link>
              );
            }
            
            // Map nav keys to their respective anchor IDs on the homepage
            const href = l === 'nav.features' ? '/#features' : l === 'nav.howItWorks' ? '/#how-it-works' : '#';
          ...
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{
                ...
                border: `1.5px solid ${showLangMenu ? TEAL : 'var(--border-color, #e2e8f0)'}`,
                background: showLangMenu ? `${TEAL}10` : 'var(--input-bg, #f8fafc)',
                cursor: 'pointer', fontSize: '0.88rem', fontWeight: 700,
                color: 'var(--text-secondary, #334155)', transition: 'all 0.2s',
            ...
            {showLangMenu && (
              <div id="lang-menu" style={{
                ...
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    onClick={() => handleLangChange(lang)} // ✅ connecté à i18n
                    style={{
                      ...
                      color: selectedLang.code === lang.code ? TEAL : 'var(--text-secondary, #334155)',
                      fontFamily: "'Nunito', sans-serif", transition: 'background 0.15s',
                    }}
                    onMouseOver={(e) => { if (selectedLang.code !== lang.code) e.currentTarget.style.background = 'var(--bg-tertiary, #f8fafc)'; }}
          ...
          {/* Séparateur */}
          <div style={{ width: 1, height: 24, background: 'var(--border-color, rgba(0,0,0,0.12))', borderRadius: 1 }} />
          ...
          {!isLoggedIn ? (
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showProfileMenu && (
                <div style={{
                  position: 'absolute', top: 55, right: 0, background: 'var(--card-bg, #fff)',
                  borderRadius: 16, boxShadow: '0 10px 30px rgba(0,0,0,0.15)',
                  padding: '8px 0', width: 200, zIndex: 200, border: '1px solid var(--border-color, #e2e8f0)',
                }}
                  onMouseLeave={() => setShowProfileMenu(false)}
                >
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color, #f1f5f9)' }}>
                    <div style={{ fontWeight: 700, color: 'var(--text-primary, #1e293b)' }}>{user?.prenom} {user?.nom}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted, #64748b)' }}>{user?.email}</div>
                  </div>
...
>
  {t('nav.settings')}
                  ...
                  <div style={{ height: 1, background: 'var(--border-color, #e2e8f0)', margin: '8px 0' }} />

                  <button onClick={() => { setShowProfileMenu(false); onLogout(); }}
                    style={{ ...menuItemStyle, color: 'var(--error-text, #ef4444)' }}>
                     {t('nav.logout')}
                  </button>
                </div>
              )}
...
const menuItemStyle = {
  ...
  fontSize: '0.95rem', color: 'var(--text-secondary, #334155)', transition: 'background 0.2s',
  display: 'flex', alignItems: 'center', gap: 10,
};