...
export default function HomePage({ user, isLoggedIn, onLoginSuccess, onLogout, openAuthModal }) {
  ...
  useEffect(() => {
    if (openAuthModal === 'signin') {
      setAuthModal({ open: true, signUp: false });
    }
  }, [openAuthModal]);

  useEffect(() => {
    const isPaymentReturn = searchParams.get('redirect_status') === 'succeeded'
      || searchParams.get('signup') === 'twint';

    if (isPaymentReturn) {
      setAuthModal({ open: true, signUp: true });
    }
  }, [searchParams]);
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'var(--bg-primary, #FDFBF5)',
      fontFamily: 'var(--font-family, "Atkinson Hyperlegible", sans-serif)',
